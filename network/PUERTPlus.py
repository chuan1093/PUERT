...
class PUERTPlus(torch.nn.Module):
    def __init__(self, LayerNo, rb_num, desired_sparsity, sparse_ratio_=0, flag_1D=False):
        ...
        basicblock = ISTA_2RB_ConcatP_BasicBlock
    ...
    def forward(self, gt):
        ...
        maskp = maskp.unsqueeze(0).unsqueeze(-1)
        mask = [mask, maskp]
...
class ISTA_2RB_ConcatP_BasicBlock(torch.nn.Module):
    def __init__(self, rb_num):
        super(ISTA_2RB_ConcatP_BasicBlock, self).__init__()
        ...
        self.conv_D = nn.Conv2d(2, n_feat, kernel_size, padding=(kernel_size//2), bias=bias)
    ...
    def forward(self, xk, PhiTb, mask):
        x_input_1 = xk - self.lambda_step * zero_filled(xk, mask[0]) + self.lambda_step * PhiTb

        x_input_2 = xk - self.lambda_step * zero_filled(xk, mask[1]) + self.lambda_step * PhiTb

        x_concat = torch.cat([x_input_1, x_input_2], dim=1)

        x_D = self.conv_D(x_concat)

        x_backward = self.body(x_D)

        x_G = self.conv_G(x_backward)

        x_pred = x_input_1 + x_G
